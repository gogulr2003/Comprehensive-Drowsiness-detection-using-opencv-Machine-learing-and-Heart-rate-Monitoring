...
while True:
    ...
    for rect in rects:
        ...
        if head_tilt_degree:
            ...
            # Check if the head tilt degree exceeds 300 degrees
            if head_tilt_degree[0] > TILT_THRESHOLD:
                tilt_counter += 1  # Increment if tilt is above threshold
            else:
                tilt_counter = 0  # Reset if tilt is below threshold

            # Trigger the alarm if the counter reaches or exceeds the limit
            if tilt_counter >= TILT_CONSECUTIVE_LIMIT:
                trigger_alarm()
